...
from utils import get_settings, get_size, is_subscribed, save_group_settings, temp, verify_user, check_token, check_verification, get_token, send_all, get_tutorial
...
async def start(client, message):
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        if settings['is_shortlink'] and user not in db.has_premium_access(message.from_user.id):
