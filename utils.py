...
async def is_subscribed(bot, query):     
    ...
    for channel in channels:
        try:
            ...
            if member.status == enums.ChatMemberStatus.BANNED or member.status == enums.ChatMemberStatus.LEFT:
                return False 
    return True
...
async def get_token(bot, userid, link, fileid):
    ...
    last_date, last_time = str((datetime(year=int(year), month=int(month), day=int(day), hour=int(hour), minute=int(minute), second=int(second)))-timedelta(hours=1)).split(" ")
